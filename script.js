document.addEventListener('DOMContentLoaded', function() {
    ...
    const filteredChartData = chartData.filter(d => d.date < firstDayOfCurrentMonth);
    ...
    const xScale = d3.scaleTime()
        .domain(d3.extent(filteredChartData, d => d.date))
        .range([0, width]);
    
    const yScale = d3.scaleLinear()
        .domain([0, d3.max(filteredChartData, d => d.value) * 1.1])
        .range([height, 0]);
    ...
    overlay.on("mousemove", (event) => {
        ...
        const i = bisectDate(filteredChartData, date, 1);
        const d0 = filteredChartData[i - 1];
        const d1 = filteredChartData[i];
        if (!d0 || !d1) return;
